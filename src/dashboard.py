import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go

st.set_page_config(page_title="İBB Deniz Ulaşım Analizi", layout="wide")

# --- KOORDİNAT LİSTESİNİ DEVASA HALE GETİRDİK ---
# İpucu: Eğer hala eksik varsa, sağdaki listede yazan ismi buraya eklemeliyiz.
ISKELE_KOORDINAT = {
    'KADIKÖY': [40.9910, 29.0232], 'EMİNÖNÜ': [41.0185, 28.9744], 'EMİNÖNÜ - ÜSKÜDAR': [41.0185, 28.9744],
    'BEŞİKTAŞ': [41.0422, 29.0083], 'ÜSKÜDAR': [41.0272, 29.0115], 'ÜSKÜDAR - EMINONU': [41.0272, 29.0115],
    'KARAKÖY': [41.0215, 28.9755], 'BOSTANCI': [40.9515, 29.0944], 'ADALAR': [40.8745, 29.1275],
    'ORTAKÖY': [41.0474, 29.0253], 'EMİRGAN': [41.1030, 29.0560], 'ARNAVUTKÖY': [41.0668, 29.0435],
    'BEBEK': [41.0772, 29.0445], 'KANLICA': [41.1010, 29.0655], 'ÇENGELKÖY': [41.0510, 29.0520],
    'KUZGUNCUK': [41.0360, 29.0305], 'ANADOLU HİSARI': [41.0830, 29.0665], 'RUMELİ HİSARI': [41.0850, 29.0570],
    'KABATAŞ': [41.0333, 28.9933], 'SARIYER': [41.1663, 29.0573], 'BEYKOZ': [41.1345, 29.0912],
    'KANDİLLİ': [41.0754, 29.0588], 'ÇUBUKLU': [41.1085, 29.0825], 'İSTİNYE': [41.1135, 29.0555],
    'PAŞABAHÇE': [41.1177, 29.0934], 'KASIMPAŞA': [41.0331, 28.9682], 'FENER': [41.0307, 28.9507],
    'BALAT': [41.0338, 28.9469], 'SÜTLÜCE': [41.0475, 28.9419], 'EYÜP': [41.0478, 28.9344],
    'HAREM': [41.0115, 29.0105], 'SİRKECİ': [41.0158, 28.9774], 'YENİKAPI': [40.9995, 28.9535],
    'MODA': [40.9785, 29.0245], 'MALTEPE': [40.9165, 29.1315], 'PENDİK': [40.8765, 29.2315]
}


@st.cache_data
def veri_yukle():
    df = pd.read_csv("data/sadece_deniz_temmuz.csv", encoding='utf-8-sig', low_memory=False)
    df['station_poi_desc_cd'] = df['station_poi_desc_cd'].astype(str).str.strip().str.upper()
    df['line_name'] = df['line_name'].astype(str).str.strip().str.upper()
    df['transition_date'] = pd.to_datetime(df['transition_date'], errors='coerce')
    df = df.dropna(subset=['transition_date'])
    df['Yıl'] = df['transition_date'].dt.year.astype(int)
    df['Ay_Gun'] = df['transition_date'].dt.day.astype(int)

    # Varış Tahmini (Standardize edilmiş)
    def varis_bul(line, origin):
        l, o = str(line).replace("-", " "), str(origin)
        parts = l.split()
        res = [p for p in parts if p not in o]
        return " ".join(res) if res else l

    df['varis_tahmini'] = df.apply(lambda x: varis_bul(x['line_name'], x['station_poi_desc_cd']),
                                   axis=1).str.strip().str.upper()
    return df


df = veri_yukle()

# --- NAVİGASYON ---
sayfa = st.sidebar.radio("Görünüm Seçin:", ["Klasik Hat Analizi", "Akış Matrisi", "Harita Üzerinde Yoğunluk"])

if sayfa == "Harita Üzerinde Yoğunluk":
    st.title("🗺️ İstanbul Boğazı Güzergah Yoğunluk Haritası")

    h_yil = st.sidebar.selectbox("Yıl", sorted(df['Yıl'].unique(), reverse=True), key="hy")
    h_gun = st.sidebar.slider("Günü Seçin", 1, 31, 5, key="hg")
    h_saat = st.sidebar.slider("Saat", 0, 23, 8, key="hs")

    # Filtrele
    h_filt_df = df[(df['Yıl'] == h_yil) & (df['Ay_Gun'] == h_gun) & (df['transition_hour'] == h_saat)]

    if not h_filt_df.empty:
        rota_df = h_filt_df.groupby(['station_poi_desc_cd', 'varis_tahmini'])['number_of_passenger'].sum().reset_index()
        rota_df = rota_df.sort_values(by='number_of_passenger', ascending=False)

        col_map, col_list = st.columns([2, 1])

        with col_map:
            fig_map = go.Figure()

            # Haritayı canlandır (Temel merkez: Boğaz)
            fig_map.update_layout(
                mapbox_style="carto-positron",
                mapbox_zoom=10.5,
                mapbox_center={"lat": 41.04, "lon": 29.02},
                margin={"r": 0, "t": 0, "l": 0, "b": 0}, height=600, showlegend=False
            )

            cizilen_rota = 0
            eksik_iskeleler = set()

            for i, row in rota_df.head(10).iterrows():
                start, end = row['station_poi_desc_cd'], row['varis_tahmini']

                if start in ISKELE_KOORDINAT and end in ISKELE_KOORDINAT:
                    s, e = ISKELE_KOORDINAT[start], ISKELE_KOORDINAT[end]
                    fig_map.add_trace(go.Scattermapbox(
                        mode="lines+markers",
                        lon=[s[1], e[1]], lat=[s[0], e[0]],
                        marker={'size': 10, 'color': 'red'},
                        line=dict(width=(row['number_of_passenger'] / 100) + 1.5, color='red'),
                        text=f"{start} ➔ {end}<br>Yolcu: {row['number_of_passenger']}",
                        hoverinfo='text'
                    ))
                    cizilen_rota += 1
                else:
                    if start not in ISKELE_KOORDINAT: eksik_iskeleler.add(start)
                    if end not in ISKELE_KOORDINAT: eksik_iskeleler.add(end)

            st.plotly_chart(fig_map, use_container_width=True)

            if cizilen_rota == 0:
                st.error("🚨 Haritada gösterilecek iskelelerin koordinatları eksik!")
                with st.expander("📍 Koordinatı Eksik Olan İskeleleri Gör"):
                    st.write(", ".join(eksik_iskeleler))
                    st.info("Lütfen bu isimleri kopyalayıp bana at, listeye ekleyelim!")

        with col_list:
            st.subheader("📋 Sefer Sıralaması")
            st.dataframe(rota_df.rename(
                columns={'station_poi_desc_cd': 'Kalkış', 'varis_tahmini': 'Varış', 'number_of_passenger': 'Yolcu'}),
                         use_container_width=True, height=550)
    else:
        st.warning("Bu tarih ve saatte veri bulunamadı.")

# (Klasik Hat ve Matris kodlarını buranın altına veya üstüne ekleyebilirsin)